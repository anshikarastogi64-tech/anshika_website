...
async function createDailyUpdate(projectId, authorType, authorId, text) {
  ...
  await run(
    'INSERT INTO portal_daily_updates (id, project_id, author_type, author_id, text, visible_to_client) VALUES (?, ?, ?, ?, ?, ?)',
    [id, projectId, authorType, authorId || null, text || null, visibleToClient]
  );
  return id;
}
...
async function updateDailyUpdateText(updateId, text) {
  await run('UPDATE portal_daily_updates SET text = ? WHERE id = ?', [text != null && String(text).trim() ? String(text).trim() : null, updateId]);
}
