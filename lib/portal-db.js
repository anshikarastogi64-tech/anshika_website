...
async function upsertClientProfile(userId, body) {
  ...
  let lat;
  let lng;
  if (existing && !has('site_map_lat') && !has('site_map_lng')) {
    lat = existing.site_map_lat;
    lng = existing.site_map_lng;
  } else {
  ...
  const siteAddr =
    existing && !has('site_location_address')
      ? String(existing.site_location_address || '').slice(0, 2000)
      : String(body.site_location_address || '').slice(0, 2000);
  const mapsLink =
    existing && !has('site_maps_link')
      ? String(existing.site_maps_link || '').slice(0, 2000)
      : String(body.site_maps_link || '').slice(0, 2000);
  const annNote =
    existing && !has('anniversary_note')
      ? String(existing.anniversary_note || '').slice(0, 500)
      : String(body.anniversary_note || '').slice(0, 500);
  const patch = {
    current_address: String(body.current_address || '').slice(0, 2000),
    site_location_address: siteAddr,
    date_of_birth: dob && dateRe.test(dob) ? dob : null,
    anniversary_date: ad && dateRe.test(ad) ? ad : null,
    anniversary_note: annNote,
    site_map_lat: lat,
    site_map_lng: lng,
    site_maps_link: mapsLink,
    ...
    work_company_name: String(body.work_company_name || '').slice(0, 200),
    occupation: String(body.occupation || '').slice(0, 200),
    job_designation: String(body.job_designation || '').slice(0, 200),
    design_style_notes: String(body.design_style_notes || '').slice(0, 2000),
    timezone_or_city: String(body.timezone_or_city || '').slice(0, 120),
  };
  if (!existing) {
    await run(
      `INSERT INTO portal_client_profiles (user_id, current_address, site_location_address, date_of_birth, anniversary_date, anniversary_note, site_map_lat, site_map_lng, site_maps_link, preferred_contact_method, emergency_contact_name, emergency_contact_phone, work_company_name, occupation, job_designation, design_style_notes, timezone_or_city, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))`,
      [
        userId,
        ...
        patch.work_company_name,
        patch.occupation,
        patch.job_designation,
    ...
    await run(
      `UPDATE portal_client_profiles SET
        ...
        emergency_contact_phone = ?, work_company_name = ?, occupation = ?, job_designation = ?, design_style_notes = ?, timezone_or_city = ?, updated_at = datetime('now')
       WHERE user_id = ?`,
      [
        patch.current_address,
        patch.site_location_address,
...
const REMINDER_CAMPAIGN_UPDATABLE = new Set([
  'status',
  'phase',
  'interval_days',
  'soft_sends_before_hard',
  'channel_email',
  'channel_sms',
  'sends_count',
  'soft_sends_count',
  'last_sent_at',
  'next_run_at',
  'last_amount_remaining',
  'stopped_at',
  'completed_at',
]);

/** The one campaign that is currently steering a project (ACTIVE or PAUSED), if any. */
async function getSteeringReminderCampaign(projectId) {
  return get(
    "SELECT * FROM portal_payment_reminder_campaigns WHERE project_id = ? AND status IN ('ACTIVE','PAUSED') ORDER BY created_at DESC LIMIT 1",
    [projectId]
  );
}

/** Most recent campaign for a project regardless of status (for showing history/state). */
async function getLatestReminderCampaign(projectId) {
  return get(
    'SELECT * FROM portal_payment_reminder_campaigns WHERE project_id = ? ORDER BY created_at DESC LIMIT 1',
    [projectId]
  );
}

async function getReminderCampaignById(id) {
  return get('SELECT * FROM portal_payment_reminder_campaigns WHERE id = ?', [id]);
}

async function createReminderCampaign(data) {
  const id = uuid();
  await run(
    `INSERT INTO portal_payment_reminder_campaigns
       (id, project_id, status, phase, interval_days, soft_sends_before_hard,
        channel_email, channel_sms, sends_count, soft_sends_count, next_run_at,
        last_amount_remaining, started_by_user_id)
     VALUES (?, ?, 'ACTIVE', 'SOFT', ?, ?, ?, ?, 0, 0, ?, ?, ?)`,
    [
      id,
      data.projectId,
      Math.max(1, Number(data.intervalDays) || 3),
      Math.max(0, Number(data.softSendsBeforeHard) || 0),
      data.channelEmail ? 1 : 0,
      data.channelSms ? 1 : 0,
      data.nextRunAt || null,
      data.lastAmountRemaining != null ? Number(data.lastAmountRemaining) : null,
      data.startedByUserId || null,
    ]
  );
  return getReminderCampaignById(id);
}

async function updateReminderCampaign(id, fields) {
  const keys = Object.keys(fields || {}).filter((k) => REMINDER_CAMPAIGN_UPDATABLE.has(k));
  if (!keys.length) return;
  const set = keys.map((k) => `${k} = ?`).join(', ');
  const params = keys.map((k) => fields[k]);
  params.push(id);
  await run(`UPDATE portal_payment_reminder_campaigns SET ${set} WHERE id = ?`, params);
}

/** ACTIVE campaigns whose next_run_at has arrived (or was never set). */
async function getDueReminderCampaigns(nowIso) {
  return all(
    "SELECT * FROM portal_payment_reminder_campaigns WHERE status = 'ACTIVE' AND (next_run_at IS NULL OR next_run_at <= ?) ORDER BY next_run_at ASC",
    [nowIso]
  );
}

/** Active campaign currently in HARD phase for a project (drives client-side gating). */
async function getActiveHardAskCampaign(projectId) {
  return get(
    "SELECT * FROM portal_payment_reminder_campaigns WHERE project_id = ? AND status = 'ACTIVE' AND phase = 'HARD' ORDER BY created_at DESC LIMIT 1",
    [projectId]
  );
}

async function addReminderLog(data) {
  const id = uuid();
  await run(
    `INSERT INTO portal_payment_reminder_log
       (id, campaign_id, project_id, phase, channel, recipient, amount_remaining, status, error)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      id,
      data.campaignId,
      ...
      data.channel,
      data.recipient || null,
      data.amountRemaining != null ? Number(data.amountRemaining) : null,
      data.status,
      data.error || null,
    ]
  );
...
async function getReminderLogsByCampaign(campaignId, limit = 30) {
  return all(
    'SELECT * FROM portal_payment_reminder_log WHERE campaign_id = ? ORDER BY created_at DESC LIMIT ?',
    [campaignId, Math.max(1, Number(limit) || 30)]
  );
}
...
async function resolveMaterialSelectionDesignLink(projectId, linkedDesignVersionId) {
  let linkVersionId =
  ...
  return { linkVersionId, linkDesignId };
}
...
async function createMaterialSelection({
  ...
  const { linkVersionId, linkDesignId } = await resolveMaterialSelectionDesignLink(projectId, linkedDesignVersionId);
  const spec = specification != null ? String(specification).trim().slice(0, 4000) : '';
  const id = uuid();
  await run(
    `INSERT INTO portal_material_selections (
       id, project_id, area_tag, linked_design_id, linked_design_version_id, material_code, image_url, file_name, specification,
...
async function updateMaterialSelection(projectId, materialId, patch) {
  const row = await get('SELECT id FROM portal_material_selections WHERE id = ? AND project_id = ?', [
    materialId,
    projectId,
  ]);
  if (!row) return { ok: false };
  const { linkVersionId, linkDesignId } = await resolveMaterialSelectionDesignLink(
    projectId,
    patch.linkedDesignVersionId
  );
  const code = (patch.materialCode || '').trim();
  if (!code) return { ok: false };
  const areaTag = (patch.areaTag || '').trim() || 'General';
  const spec = patch.specification != null ? String(patch.specification).trim().slice(0, 4000) : '';
  if (patch.imageUrl) {
    await run(
      `UPDATE portal_material_selections SET
         area_tag = ?, linked_design_id = ?, linked_design_version_id = ?, material_code = ?, image_url = ?, file_name = ?, specification = ?,
         client_status = 'PENDING', client_note = NULL
       WHERE id = ? AND project_id = ?`,
      [areaTag, linkDesignId, linkVersionId, code, patch.imageUrl, patch.fileName || null, spec, materialId, projectId]
    );
  } else {
    await run(
      `UPDATE portal_material_selections SET
         area_tag = ?, linked_design_id = ?, linked_design_version_id = ?, material_code = ?, specification = ?,
         client_status = 'PENDING', client_note = NULL
       WHERE id = ? AND project_id = ?`,
      [areaTag, linkDesignId, linkVersionId, code, spec, materialId, projectId]
    );
  }
  return { ok: true };
}
...
// ----- Client design inspiration / ideas (per project, areas + uploads + comments) -----
async function listDesignIdeaAreasForProject(projectId) {
  return all(
    `SELECT * FROM portal_project_idea_areas WHERE project_id = ? ORDER BY name COLLATE NOCASE ASC`,
    [projectId]
  );
}

async function getDesignIdeaAreaInProject(areaId, projectId) {
  if (!areaId) return null;
  return get(`SELECT * FROM portal_project_idea_areas WHERE id = ? AND project_id = ?`, [areaId, projectId]);
}

async function addDesignIdeaAreaForProject(projectId, name) {
  const t = String(name || '').trim().slice(0, 120);
  if (!t) return null;
  const existing = await get(
    `SELECT id FROM portal_project_idea_areas WHERE project_id = ? AND LOWER(TRIM(name)) = LOWER(?)`,
    [projectId, t]
  );
  if (existing) return existing.id;
  const id = uuid();
  await run(`INSERT INTO portal_project_idea_areas (id, project_id, name) VALUES (?, ?, ?)`, [id, projectId, t]);
  return id;
}

async function createDesignIdea(projectId, authorUserId, payload) {
  const areaId = payload.areaId || null;
  const id = uuid();
  await run(
    `INSERT INTO portal_project_client_ideas (id, project_id, author_user_id, area_id, title, note_text, link_url, file_path, file_name, file_type)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      id,
      projectId,
      authorUserId,
      areaId,
      String(payload.title || '').trim().slice(0, 200),
      String(payload.noteText || '').trim().slice(0, 8000),
      String(payload.linkUrl || '').trim().slice(0, 2000),
      payload.filePath || null,
      payload.fileName ? String(payload.fileName).slice(0, 500) : null,
...
async function getDesignIdeaByIdInProject(ideaId, projectId) {
  return get(`SELECT * FROM portal_project_client_ideas WHERE id = ? AND project_id = ?`, [ideaId, projectId]);
}

async function listDesignIdeasWithCommentsForProject(projectId) {
  const ideas = await all(
    `SELECT i.*, a.name AS area_name, u.full_name AS author_name, u.role AS author_role
     FROM portal_project_client_ideas i
     LEFT JOIN portal_project_idea_areas a ON a.id = i.area_id
     JOIN portal_users u ON u.id = i.author_user_id
     WHERE i.project_id = ?
     ORDER BY datetime(i.created_at) DESC`,
    [projectId]
  );
  if (!ideas.length) return [];
  const ids = ideas.map((x) => x.id);
  const ph = ids.map(() => '?').join(',');
  const comments = await all(
    `SELECT c.*, u.full_name AS author_name, u.role AS author_role
     FROM portal_project_client_idea_comments c
     JOIN portal_users u ON u.id = c.author_user_id
     WHERE c.idea_id IN (${ph})
     ORDER BY datetime(c.created_at) ASC`,
    ids
  );
  const by = {};
  comments.forEach((c) => {
    if (!by[c.idea_id]) by[c.idea_id] = [];
    by[c.idea_id].push(c);
  });
  return ideas.map((i) => ({ ...i, comments: by[i.id] || [] }));
}

async function addDesignIdeaComment(ideaId, authorUserId, body) {
  const t = String(body || '').trim();
  if (!t) return null;
  const id = uuid();
  await run(
    `INSERT INTO portal_project_client_idea_comments (id, idea_id, author_user_id, body) VALUES (?, ?, ?, ?)`,
    [id, ideaId, authorUserId, t.slice(0, 8000)]
  );
  return id;
}
...
module.exports = {
  ...
  deleteClientPayment,
  getSteeringReminderCampaign,
  getLatestReminderCampaign,
  getReminderCampaignById,
  createReminderCampaign,
  updateReminderCampaign,
  getDueReminderCampaigns,
  getActiveHardAskCampaign,
  addReminderLog,
  getReminderLogsByCampaign,
  getQuotationById,
  ...
  updateMeetingStatus,
  listDesignIdeaAreasForProject,
  getDesignIdeaAreaInProject,
  addDesignIdeaAreaForProject,
  createDesignIdea,
  getDesignIdeaByIdInProject,
  listDesignIdeasWithCommentsForProject,
  addDesignIdeaComment,
};
