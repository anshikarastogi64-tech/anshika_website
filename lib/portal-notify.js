...
const DEFAULT_ROUTING = {
  ...
  MESSAGE: { notify_client: 1, notify_admin: 1, notify_designer: 1 },
  MEETING: { notify_client: 1, notify_admin: 1, notify_designer: 1 },
};
...
async function notifyProjectStakeholders(db, params) {
  ...
  if (clientEligible && routing.notify_client) {
    const clientIds = await db.getProjectClientRecipientUserIds(projectId);
  ...
  if (shouldNotifyDesigner(category, routing)) {
    const designerIds = await db.getProjectDesignerRecipientUserIds(projectId);
